...
const iwcoingeckoApp = cutil.extend(iwcoingecko, {
	...
	async toDefineInitOptionsIWCoingeckoApp() {
		...
		app.commander.option("--set-price <symbolAndPrice>", "Set price for symbol persistently");
		app.commander.option("--del-price <symbol>", "Remove price for symbol persistently");
		app.commander.option("--cg-id <symbolAndCgId>", "Set cgId for symbol");
		app.commander.option("--set-cg-id <symbolAndCgId>", "Set cgId for symbol persistently");
		app.commander.option("--del-cg-id <symbol>", "Remove cgId for symbol persistently");
	},
	async toApplyInitOptionsIWCoingeckoApp() {
		...
		if (cutil.a(opts.price)) {
			app._defPrices = {
				...
				...Object.fromEntries(
					cutil.asString(opts.price).split(",")
					.map(part => part.split("="))
		...
		if (cutil.a(opts.setPrice)) {
			app.prefs.defPrices = {
				...
				...Object.fromEntries(
					cutil.asString(opts.setPrice).split(",")
					.map(part => part.split("="))
					.map(([symbol, price]) => ([symbol, cutil.asNumber(price)]))
				),
			};
		...
		if (cutil.a(opts.cgId)) {
			app._cgIds = {
				...
				...Object.fromEntries(
					cutil.asString(opts.cgId).split(",")
					.map(part => part.split("="))
				),
			};
		}
		if (cutil.a(opts.setCgId)) {
			app.prefs.cgIds = {
				...
				...Object.fromEntries(
					cutil.asString(opts.setCgId).split(",")
					.map(part => part.split("="))
				),
			};
			app.prefs.save();
